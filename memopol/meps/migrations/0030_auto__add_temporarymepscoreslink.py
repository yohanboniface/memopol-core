...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding model 'TemporaryMEPScoresLink'
        db.create_table('meps_temporarymepscoreslink', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('representative', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['meps.MP_EU'])),
            ('score', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['votes.Score'])),
        ))
        db.send_create_signal('meps', ['TemporaryMEPScoresLink'])

        total = orm["votes.score"].objects.filter(representative__mep__isnull=False).count()
        for number, score in enumerate(orm["votes.score"].objects.filter(representative__mep__isnull=False), 1):
            sys.stdout.write("scores %s/%s\r" % (number, total))
            sys.stdout.flush()
            orm["meps.temporarymepscoreslink"].objects.create(
                representative=orm["meps.MP_EU"].objects.get(remote_id=score.representative.mep.ep_id),
                score=score,
            )
        sys.stdout.write("\n")
    ...
    def backwards(self, orm):
        ...
        # Deleting model 'TemporaryMEPScoresLink'
        db.delete_table('meps_temporarymepscoreslink')
