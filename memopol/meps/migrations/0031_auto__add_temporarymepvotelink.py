...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding model 'TemporaryMEPVoteLink'
        db.create_table('meps_temporarymepvotelink', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('representative', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['meps.MP_EU'])),
            ('vote', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['votes.Vote'])),
        ))
        db.send_create_signal('meps', ['TemporaryMEPVoteLink'])

        total = orm["votes.vote"].objects.filter(representative__mep__isnull=False).count()
        for number, vote in enumerate(orm["votes.vote"].objects.filter(representative__mep__isnull=False), 1):
            sys.stdout.write("votes %s/%s\r" % (number, total))
            sys.stdout.flush()
            orm["meps.temporarymepvotelink"].objects.create(
                representative=orm["meps.MP_EU"].objects.get(remote_id=vote.representative.mep.ep_id),
                vote=vote,
            )
        sys.stdout.write("\n")
    ...
    def backwards(self, orm):
        ...
        # Deleting model 'TemporaryMEPVoteLink'
        db.delete_table('meps_temporarymepvotelink')
    ...
    models = {
        ...
        'meps.temporarymepscoreslink': {
            'Meta': {'object_name': 'TemporaryMEPScoresLink'},
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'representative': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['meps.MP_EU']"}),
            'score': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['votes.Score']"})
        },
        'meps.temporarymepvotelink': {
            'Meta': {'object_name': 'TemporaryMEPVoteLink'},
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'representative': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['meps.MP_EU']"}),
            'vote': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['votes.Vote']"})
        },
